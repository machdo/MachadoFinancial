...
export default function Categories({
  ...
  const categoryStats = useMemo(() => {
    ...
    for (const transaction of transactions) {
      const current = map.get(transaction.categoryId) ?? { count: 0, expenseTotal: 0 };
      current.count += 1;
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950">
        ...
        <div className="space-y-2">
          {categories.map((category) => {
            const stats = categoryStats.get(category.id) ?? { count: 0, expenseTotal: 0 };
            const isEditing = editingId === category.id;
            const isRowBusy = rowBusyId === category.id;
            const storedDescriptions = descriptionMap[String(category.id)] ?? [];
            const suggestions = getDefaultDescriptionSuggestions(category.name).filter(
              (item) =>
                        ...
                        <div className="mt-2 flex flex-wrap gap-2">
                          {storedDescriptions.map((item) => (
                            <button
                              key={item}
                              type="button"
                              onClick={() => handleRemoveDescription(category.id, item)}
